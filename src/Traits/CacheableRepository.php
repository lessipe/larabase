...
trait CacheableRepository
{
    ...
    public function getCacheRepository()
    {
        if (is_null($this->cacheRepository)) {
            $this->cacheRepository = app(config('larabase.cache.repository', 'cache'));
        }
    ...
    public function isSkippedCache()
    {
        ...
        $skipCacheParam = config('larabase.cache.params.skipCache', 'skipCache');
    ...
    {
        $cacheEnabled = config('larabase.cache.enabled', true);

        if (!$cacheEnabled) {
            return false;
        }

        $cacheOnly = isset($this->cacheOnly) ? $this->cacheOnly : config('larabase.cache.allowed.only', null);
        $cacheExcept = isset($this->cacheExcept) ? $this->cacheExcept : config('larabase.cache.allowed.except', null);
    ...
    {
        $cacheMinutes = isset($this->cacheMinutes) ? $this->cacheMinutes : config('larabase.cache.minutes', 30);
