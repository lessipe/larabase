...
abstract class Generator
{
    ...
    {
        $path = config('larabase.generator.stubsOverridePath', __DIR__);
   ...
   /**
    ...
    {
        return config('larabase.generator.rootNamespace', $this->getAppNamespace());
    }
    ...
    public function getConfigGeneratorClassPath($class, $directoryPath = false)
    {
        switch ($class) {
            case ('mails' === $class):
                $path = config('larabase.generator.paths.mails', 'Mails');
                break;
            ...
            case ('composers' === $class):
                ...
                break;
            case ('policies' === $class):
                $path = config('larabase.generator.paths.policies', 'Policies');
                break;
            case ('actions' === $class):
                $path = config('larabase.generator.paths.actions', 'Actions');
                break;
            case ('jobs' === $class):
                $path = config('larabase.generator.paths.jobs', 'Jobs');
                break;
            case ('services' === $class):
                $path = config('larabase.generator.paths.services', 'Services');
                break;
            case ('models' === $class):
                $path = config('larabase.generator.paths.models', 'Entities');
                break;
            case ('repositories' === $class):
                $path = config('larabase.generator.paths.repositories', 'Repositories');
                break;
            case ('interfaces' === $class):
                $path = config('larabase.generator.paths.interfaces', 'Repositories');
                break;
            case ('presenters' === $class):
                $path = config('larabase.generator.paths.presenters', 'Presenters');
                break;
            case ('transformers' === $class):
                $path = config('larabase.generator.paths.transformers', 'Transformers');
                break;
            case ('validators' === $class):
                $path = config('larabase.generator.paths.validators', 'Validators');
                break;
